...
class ListaEstudios(ListView):
    ...
    template_name = 'entrevista/estudio/lista_estudios_entrevista.html'
...
class CrearEstudio(CreateView):
    ...
    def form_valid(self, form):
        messages.add_message(self.request, messages.SUCCESS, 'Estudio ' + form.clean_titulo() + ' registrado.')
...
class EditarEstudio(UpdateView):
    ...
    def form_valid(self, form):
        messages.add_message(self.request, messages.SUCCESS, 'Estudio ' + form.clean_titulo() + ' actualizado.')
...
class ConsultarEstudio(DetailView):
    ...
    model = EstudioEntrevista
    template_name = 'entrevista/estudio/consultar_estudio.html'
    context_object_name = 'estudio'

    def get_context_data(self, **kwargs):
        context = super(ConsultarEstudio, self).get_context_data(**kwargs)
        estudio = get_object_or_404(EstudioEntrevista, id=self.kwargs['pk'])
        context['usuario'] = obtener_tipo_usuario(self, estudio.id)
        return context
...
class ListaPreguntas(ListView):
    ...
    def get_queryset(self):
        ...
        actualizar_rondas(self.estudio.id)
    ...
    def get_context_data(self, **kwargs):
        ...
        rondas_activas = RondaJuicio.objects.filter(idEstudio=self.estudio.id, estado=True).count()
...
class EditarPregunta(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['usuario'] = obtener_tipo_usuario(self, pregunta.idEstudio.id)
...
class ConsultarPregunta(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        pregunta = get_object_or_404(Pregunta, id=self.kwargs['pk'])
        estudio = get_object_or_404(EstudioEntrevista, id=pregunta.idEstudio.id)
        context['usuario'] = obtener_tipo_usuario(self, estudio.id)
        return context
...
class ListaValoresEscala(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        rondas = RondaJuicio.objects.filter(idEstudio=self.estudio.id)
        rondas_activas = rondas.filter(estado=True).count()
        context['estudio'] = self.estudio
        context['ronda_activa'] = rondas_activas
...
class CrearValorEscala(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        valores_registrados = ValorEscalaLikert.objects.filter(idEstudio=estudio.id).order_by('-valor')
    ...
    def form_invalid(self, form):
        ...
        messages.error(self.request, 'La alternativa de respuesta no pudo ser registrada. Ingrese datos válidos. Verifique'
                                     ' que el nombre o valor ingresados no se encuentren ya en la escala de Likert del'
                                     ' estudio. ')
        return response
...
class EditarValorEscala(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        estudio = get_object_or_404(EstudioEntrevista, id=valor.idEstudio.id)
        context['estudio'] = estudio
        context['usuario'] = obtener_tipo_usuario(self, estudio.id)
        context['rondas_activas'] = RondaJuicio.objects.filter(idEstudio=estudio.id, estado=True).count()
        return context
...
class ListaRondas(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['usuario'] = obtener_tipo_usuario(self, self.estudio.id)
        context['hoy'] = date.today()
        context['rondas_activas'] = RondaJuicio.objects.filter(idEstudio=self.estudio.id, estado=True).count()
        actualizar_rondas(self.estudio.id)
        return context
...
class CrearRonda(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        preguntas = Pregunta.objects.filter(idEstudio=estudio.id).count()
        escala = ValorEscalaLikert.objects.filter(idEstudio=estudio.id).count()

        if len(rondas) > 0:
            rondas_activa = rondas.filter(estado=True).count()
...
class EditarRonda(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        rondas_activas = RondaJuicio.objects.filter(idEstudio=estudio.id, estado=True).exclude(id=ronda.id)

        if len(rondas_activas) > 0:
            context['last_ronda'] = True
        else:
            context['last_ronda'] = False
        context['hoy'] = date.today()
        context['estudio'] = estudio
        context['ronda'] = ronda
        context['usuario'] = obtener_tipo_usuario(self, estudio.id)
...
class ListaJuicios(ListView):
    ...
    def get_queryset(self):
        self.estudio = get_object_or_404(EstudioEntrevista, id=self.args[0])
        actualizar_rondas(self.estudio.id)
        usuario = obtener_tipo_usuario(self, self.estudio.id)
        if usuario == "EXPERTO":
            return Juicio.objects.filter(idRonda__idEstudio=self.estudio.id, idExperto=self.request.user).\
                    order_by('-idRonda', 'texto_pregunta')
        else:
            return Juicio.objects.filter(idRonda__idEstudio=self.estudio.id).order_by('-idRonda', 'texto_pregunta')

    def get_context_data(self, **kwargs):
        ronda_activa = RondaJuicio.objects.filter(idEstudio=self.estudio.id, estado=True)
        context = super(ListaJuicios, self).get_context_data(**kwargs)
        context['estudio'] = self.estudio
        if ronda_activa.count() > 0:
            context['ronda_activa'] = ronda_activa[0]
            context['porcentaje'] = calcular_porcentaje_juicios(self, ronda_activa[0])
        context['usuario'] = obtener_tipo_usuario(self, self.estudio.id)
        return context
...
class CrearJuicio(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        estudio = get_object_or_404(EstudioEntrevista, id=self.args[0])
        preguntas = Pregunta.objects.filter(idEstudio=estudio.id).order_by('id')
        ...
        # filtro de preguntas en juicios
...
class EditarJuicio(UpdateView):
    ...
    def form_valid(self, form):
        messages.add_message(
            self.request, messages.SUCCESS, 'Juicio actualizado con exito.')
        return super(EditarJuicio, self).form_valid(form)

    def form_invalid(self, form):
        response = super(EditarJuicio, self).form_invalid(form)
        messages.error(
            self.request, 'El juicio no pudo ser actualizado.')
        return response
...
class MatrizJuicios(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if len(matriz) > 1:
            ...
            context['matriz'] = 0
            context['cant_expertos'] = str(0) + "/" + str(len(estudio.idExpertos.all()))
...
class ListaCoeficientes(ListView):
    ...
    def get_queryset(self):
        self.estudio = get_object_or_404(EstudioEntrevista, id=self.args[0])
        actualizar_rondas(self.estudio.id)
        coeficientes = CoeficienteAlfa.objects.filter(idRonda__idEstudio=self.estudio.id).order_by('idRonda__numero_ronda')
        return coeficientes
...
class CoeficienteAlfaC(TemplateView):

    template_name = 'entrevista/coeficiente/coeficiente.html'

    def get_context_data(self, **kwargs):
        context = super(CoeficienteAlfaC, self).get_context_data(**kwargs)
        ronda = get_object_or_404(RondaJuicio, id=self.args[0])
        estudio = get_object_or_404(EstudioEntrevista, id=ronda.idEstudio.id)
        matriz_juicios = calcular_matriz_juicios(estudio, ronda, "")
        context['estudio'] = estudio
        context['coeficiente'] = calcular_coeficiente(ronda, matriz_juicios, "")
        context['usuario'] = obtener_tipo_usuario(self, estudio.id)
        context['ronda'] = ronda
...
# Vizualiza la lista de promedios de juicios alcanzado para cada pregunta junto al coeficiente condicional
class TablaPromedios(TemplateView):

    template_name = 'entrevista/coeficiente/lista_promedios.html'
    paginate_by = 15

    def get_context_data(self, **kwargs):
        context = super(TablaPromedios, self).get_context_data(**kwargs)
        ronda = get_object_or_404(RondaJuicio, id=self.args[0])
        estudio = get_object_or_404(EstudioEntrevista, id=ronda.idEstudio.id)
        context['estudio'] = estudio
...
class Celda:
    def __init__(self, columna, valor, descripcion):
        self.columna = columna
        self.valor = valor
        self.descripcion = descripcion


# Determina el tipo de usuario en sesión según el estudio seleccionado
...
def obtener_estudios_usuario(self):
    ...
    estudios = EstudioEntrevista.objects.all().order_by('-estado', 'titulo')
    estudios_usuario = []
...
def calcular_matriz_juicios(estudio, ronda, pregunta_excluida):
    ...
    if len(juicios) > 0:
        for i in juicios:
            fila.append(Celda(len(fila) + 1, i.idValorEscala.valor, i.idValorEscala.nombre))
...
def calcular_porcentaje_juicios(self, ronda):
    ...
    preguntas = Pregunta.objects.filter(idEstudio=ronda.idEstudio.id).count()
...
# Actualiza el estado de las rondas, segun la fecha actual
def actualizar_rondas(idEstudio):
    ...
    for ronda in rondas:
        ...
        if ronda.estado is True and date.today() > ronda.fecha_final:
            ronda.estado = False
            ronda.save()
...
# Registra o actualiza el coeficiente Alfa de Cronbach de la ronda a medida que se desarrolla.
...
def exportar_estudio_xls(request, idEstudio):
    ...
    hoja_rondas = wb.add_sheet('Rondas')
    ...
    columns3 = ['Número', 'Descripción', 'Inicio', 'Cierre', 'Preguntas', 'Estado']
    ...
    for col_num in range(len(columns3)):
        hoja_rondas.write(row_num, col_num, columns3[col_num], font_style)
    ...
    row_num = 0
    for row in filas['rondas']:
        row_num += 1
        for col_num in range(len(row)):
            hoja_rondas.write(row_num, col_num, row[col_num], font_style)
...
def obtener_datos_estudio(request, estudio):
    ...
    filas_preguntas = Pregunta.objects.filter(idEstudio=estudio.id).values_list(
                        'texto_pregunta', 'texto_respuesta', 'observacion').order_by('id')

    filas_escala = ValorEscalaLikert.objects.filter(idEstudio=estudio.id).values_list(
                    'nombre', 'valor', 'descripcion').order_by('-valor')

    filas_rondas = RondaJuicio.objects.filter(idEstudio=estudio.id).values_list(
                    'numero_ronda', 'descripcion', 'fecha_inicio', 'fecha_final', 'numero_preguntas',
                    'estado').order_by('numero_ronda')
    ...
    filas = {'preguntas': filas_preguntas, 'escala': filas_escala, 'rondas': filas_rondas,
             'juicios': filas_juicios, 'coeficientes': filas_coeficientes}
